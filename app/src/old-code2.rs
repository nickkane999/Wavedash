...
pub fn new_field<T>(field: InputData<T>) -> Padding<FunnyData, Align<FunnyData>> {
    let text_input: LensWrap<FunnyData, String, T, Option<T>>  = match field.field {
        FunnyDataOptions::email_address => TextBox::new().with_placeholder(field.text).lens(FunnyData::email_address),
        FunnyDataOptions::email_subject => TextBox::new().with_placeholder(field.text).lens(FunnyData::email_subject),
